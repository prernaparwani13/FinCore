...
const [activeButton, setActiveButton] = useState<string | null>(null);
  ...
  return (
    <div className="min-h-screen bg-slate-100 dark:bg-slate-950 pb-1 font-sans text-slate-900 dark:text-white">
      <Navbar />
    <div className="min-h-screen bg-slate-100 dark:bg-[#0f172a] transition-colors duration-300 py-10">
...
<div className="grid grid-cols-1 md:grid-cols-3 gap-8 items-center">
  {plans.map((plan) => {
    ...
    return (
      <div
        ...
        className={`
          relative p-10 rounded-[2.5rem] border cursor-pointer
      ...
      >
          ...
          className={`
            ...
            ${
              isButtonActive
                ...
                : "bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-white hover:bg-slate-50"
            }
          `}
        >
          {plan.button}
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                {comparisonFeatures.map((feature, idx) => (
                  <tr key={idx} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/50 transition-colors">
                    <td className="p-8 text-[14px] font-medium text-slate-700 dark:text-slate-300">
                      {feature.name}
                    </td>
                    
                    {/* Basic Column */}
                    <td className="p-8 text-center text-[13px] text-slate-300">
                      {renderCell(feature.basic)}
                    </td>
                    
                    {/* Pro Column */}
                    <td className="p-8 text-center text-[13px] font-bold text-blue-600 dark:text-blue-400">
                      {renderCell(feature.pro)}
                    </td>
                    
                    {/* Team Column */}
                    <td className="p-8 text-center text-[13px] text-slate-500 dark:text-slate-300">
        ...
        <section className="mt-40 max-w-6xl mx-auto px-1">
  <div className="text-center mb-16 ">
  ...
  <div className="space-y-7">
    {faqs.map((faq, index) => (
      <div
        key={index}
        className="w-full bg-white dark:bg-slate-900/50 border border-slate-100 dark:border-slate-800 rounded-3xl px-10 py-6 transition-all hover:border-blue-500/30"
      >
        ...
        <section className="max-w-[1400px] mx-auto px-15 py-20 mb-10 mt-15 -mr-13 -ml-14 bg-white dark:bg-slate-900">
          <div className="bg-blue-50 dark:bg-slate-900/50 border border-slate-100 dark:border-slate-800 rounded-[3rem] px-25 md:py-20 text-center">
            <div className="w-16 h-16 bg-white dark:bg-blue-100/30 rounded-2xl flex items-center justify-center mx-auto mb-8">
