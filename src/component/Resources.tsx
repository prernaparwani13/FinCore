...
const Resources: React.FC = () => {
  const articles = [
    { title: "Understanding Compound Interest", tag: "GUIDE", read: "5 min read", date: "Oct 12, 2024" },
    { title: "How to Pay Off Mortgage Faster", tag: "TIPS", read: "8 min read", date: "Nov 05, 2024" },
    { title: "2024 Tax Bracket Changes", tag: "TAX", read: "10 min read", date: "Dec 01, 2024" },
    { title: "ETF vs Mutual Funds: A Comparison", tag: "INVESTING", read: "6 min read", date: "Jan 15, 2025" },
    { title: "How Much Do You Really Need?", tag: "RETIREMENT", read: "12 min read", date: "Feb 20, 2025" },
    { title: "The Snowball Method Explained", tag: "DEBT", read: "4 min read", date: "Mar 10, 2025" },
  ];
    ...
    <div className="min-h-screen bg-slate-100 dark:bg-[#0f172a] transition-colors  duration-300">
      ...
      <section className="max-w-7xl mx-auto px-14 mb-32">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {articles.map((art, i) => (
  <div key={i} className="group cursor-pointer bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-[2.5rem] overflow-hidden transition-all hover:shadow-xl hover:shadow-slate-200/50">
    {/* 1. Reduced Aspect Ratio from 4/3 to 16/10 for a shorter image area */}
    <div className="aspect-[16/10] bg-slate-50 dark:bg-slate-800/50 m-3 rounded-[2rem] flex items-center justify-center transition-transform group-hover:scale-[1.01]">
    ...
    <div className="px-7 pb-7 pt-2">
      ...
      <h3 className={`text-2xl font-bold mb-2 leading-tight transition-colors ${art.highlight ? 'text-blue-600' : 'text-[#0f172a] dark:text-white group-hover:text-blue-600'}`}>
        {art.title}
      </h3>
      ...
      <section className="max-w-7xl mx-auto px-14 mb-32">
        <div className="flex justify-between items-end mb-10">
          <div>
            <h2 className="text-2xl font-bold text-[#0f172a] dark:text-white mb-2">Downloadable Assets</h2>
            <p className="text-slate-500 dark:text-slate-300 text-sm">Excel templates and PDF checklists to use offline.</p>
          </div>
          <button className="px-5 py-2 border border-slate-200 dark:border-slate-700 rounded-xl text-xs font-bold text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors">
            View All
          </button>
        </div>
      ...
      <section className="max-w-7xl mx-auto px-14 pb-32">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          <div className="relative aspect-video rounded-[2.5rem] overflow-hidden group cursor-pointer">
            ...
            <div className="absolute inset-0 bg-slate-200 dark:bg-slate-800" /> {/* Placeholder for image */}
...
{/* 4. Newsletter Section — FULL SCREEN FIXED */}
<section
  className="
    w-full
